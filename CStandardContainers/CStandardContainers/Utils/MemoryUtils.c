...
#if CSC_MEMORY_UTILS_USE_SIZE_BASED_OPTIMIZATION == FALSE || CSC_ARCH < CSC_ARCH_16BIT
...
{
	CSC_SIZE_T iterator;
	CSC_BOOLEAN reverseDir = ((CSC_POINTER_TYPE)pDst > (CSC_POINTER_TYPE)pSrc) ? (CSC_BOOLEAN)TRUE : (CSC_BOOLEAN)FALSE;
...
static CSC_STATUS CSCAPI CSC_MemoryUtilsCompareMemoryBasic(_In_ CONST CSC_PCVOID pFirst, _In_ CONST CSC_PCVOID pSecond, _In_ CONST CSC_SIZE_T size)
{
	CSC_SIZE_T iterator;
...
typedef enum _CSC_SizeType
{
	...
	csc_st_word,
#if CSC_ARCH > CSC_ARCH_16BIT
	csc_st_dword,
#if CSC_ARCH > CSC_ARCH_32BIT
	csc_st_qword
#endif
#endif
#endif
...
static CSC_STATUS CSCAPI CSC_MemoryUtilsMemMove(_When_(return == CSC_STATUS_SUCCESS, _Out_) CONST CSC_PVOID pDst, _In_ CONST CSC_PCVOID pSrc, _In_ CONST CSC_SIZE_T size, _In_ CONST CSC_SizeType sizeType)
{
	...
	prefixLength = ((reverseDir) ? ((CSC_SIZE_T)pDst + size) : (elementSize - ((CSC_SIZE_T)pDst % elementSize))) % elementSize;
...
static CSC_STATUS CSCAPI CSC_MemoryUtilsMemMoveNonRecursive(_When_(return == CSC_STATUS_SUCCESS, _Out_) CONST CSC_PVOID pDst, _In_ CONST CSC_PCVOID pSrc, _In_ CONST CSC_SIZE_T size, _In_ CONST CSC_SizeType sizeType)
{
	...
	if (prefixLength)
	{
		for (iterator = (CSC_SIZE_T)0; iterator < prefixLength; ++iterator)
		{
			if (reverseDir)
				((CSC_BYTE* CONST)pChunkDst + chunkLength)[prefixLength - iterator - (CSC_SIZE_T)1] = ((CONST CSC_BYTE* CONST)pChunkSrc + chunkLength)[prefixLength - iterator - (CSC_SIZE_T)1];
			else
				((CSC_BYTE* CONST)pDst)[iterator] = ((CONST CSC_BYTE* CONST)pSrc)[iterator];
		}
	}
	...
	if (suffixLength)
	{
		for (iterator = (CSC_SIZE_T)0; iterator < suffixLength; ++iterator)
		{
			if (reverseDir)
				((CSC_BYTE* CONST)pDst)[suffixLength - iterator - (CSC_SIZE_T)1] = ((CONST CSC_BYTE* CONST)pSrc)[suffixLength - iterator - (CSC_SIZE_T)1];
			else
				((CSC_BYTE* CONST)pChunkDst + chunkLength)[iterator] = ((CONST CSC_BYTE* CONST)pChunkSrc + chunkLength)[iterator];
		}
	}
...
static CSC_STATUS CSCAPI CSC_MemoryUtilsMemSet(_When_(return == CSC_STATUS_SUCCESS, _Out_) CONST CSC_PVOID pDst, _In_ CONST CSC_BYTE value, _In_ CONST CSC_SIZE_T size, _In_ CONST CSC_SizeType sizeType)
{
	...
	prefixLength = (elementSize - ((CSC_SIZE_T)pDst % elementSize)) % elementSize;
	suffixLength = (size - prefixLength) % elementSize;
	chunkLength = size - prefixLength - suffixLength;
...
static CSC_STATUS CSCAPI CSC_MemoryUtilsMemSetNonRecursive(_When_(return == CSC_STATUS_SUCCESS, _Out_) CONST CSC_PVOID pDst, _In_ CONST CSC_BYTE value, _In_ CONST CSC_SIZE_T size, _In_ CONST CSC_SizeType sizeType)
{
	...
	{
		for (iterator = (CSC_SIZE_T)0; iterator < suffixLength; ++iterator)
			((CSC_BYTE* CONST)pChunkDst + chunkLength)[iterator] = value;
	}
...
static CSC_STATUS CSCAPI CSC_MemoryUtilsCompareMemoryRecursive(_In_ CONST CSC_PCVOID pFirst, _In_ CONST CSC_PCVOID pSecond, _In_ CONST CSC_SIZE_T size, _In_ CONST CSC_SizeType sizeType)
{
	...
	prefixLength = (elementSize - ((CSC_SIZE_T)pFirst % elementSize)) % elementSize;
	suffixLength = (size - prefixLength) % elementSize;
	chunkLength = size - prefixLength - suffixLength;
...
static CSC_STATUS CSCAPI CSC_MemoryUtilsCompareMemoryNonRecursive(_In_ CONST CSC_PCVOID pFirst, _In_ CONST CSC_PCVOID pSecond, _In_ CONST CSC_SIZE_T size, _In_ CONST CSC_SizeType sizeType)
{
	...
	if (suffixLength)
	{
		for (iterator = (CSC_SIZE_T)0; iterator < suffixLength; ++iterator)
		{
			if (((CONST CSC_BYTE* CONST)pChunkFirst + chunkLength)[iterator] != ((CONST CSC_BYTE* CONST)pChunkSecond + chunkLength)[iterator])
				return CSC_STATUS_GENERAL_FAILURE;
		}
	}
...
CSC_STATUS CSCAPI CSC_MemoryUtilsSwapValues(_Inout_ CONST CSC_PVOID pFirst, _Inout_ CONST CSC_PVOID pSecond, _In_ CONST CSC_SIZE_T size, _In_ CONST CSC_PVOID pAllocator)
{
	...
	if (size > (CSC_SIZE_T)0x100)
		return CSC_MemoryUtilsSwapValuesHeap(pFirst, pSecond, size, pAllocator);
...
CSC_STATUS CSCAPI CSC_MemoryUtilsSwapValuesHeap(_Inout_ CONST CSC_PVOID pFirst, _Inout_ CONST CSC_PVOID pSecond, _In_ CONST CSC_SIZE_T size, _In_ CONST CSC_PVOID pAllocator)
{
	...
	if (!pFirst || !pSecond || !size || !pAllocator)
		return CSC_STATUS_INVALID_PARAMETER;
	...
	pBuffer = CSC_AllocatorAlloc(pAllocator, size);
	...
	if (status != CSC_STATUS_SUCCESS)
	{
		CSC_AllocatorFree(pAllocator, pBuffer);
		return status;
	}
...
CSC_STATUS CSCAPI CSC_MemoryUtilsMoveValue(_Out_ CONST CSC_PVOID pDst, _Inout_ CONST CSC_PVOID pSrc, _In_ CONST CSC_SIZE_T size)
{
	...
	return CSC_MemoryUtilsSetZeroMemory(pSrc, size);
}
