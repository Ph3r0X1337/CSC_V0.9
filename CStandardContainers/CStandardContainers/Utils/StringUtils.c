...
CSC_STATUS CSCAPI CSC_StringUtilsCompareWideString(_In_ CONST CSC_WCHAR* CONST pFirst, _In_ CONST CSC_WCHAR* CONST pSecond, _In_ CONST CSC_SIZE_T maxCodePoints)
{
	...
	if (!pFirst || !pSecond || maxCodePoints > CSC_MAXIMUM_STRING_LENGTH_WIDE_STRING)
		return CSC_STATUS_INVALID_PARAMETER;

	lengthFirst = CSC_StringUtilsStrLenWideString(pFirst, CSC_MAXIMUM_STRING_LENGTH_WIDE_STRING, (CSC_BOOLEAN)TRUE);
	lengthSecond = CSC_StringUtilsStrLenWideString(pSecond, CSC_MAXIMUM_STRING_LENGTH_WIDE_STRING, (CSC_BOOLEAN)TRUE);

	if (lengthFirst == CSC_STRING_INVALID_LENGTH || lengthSecond == CSC_STRING_INVALID_LENGTH)
		return CSC_STATUS_INVALID_PARAMETER;

	compareLength = ((lengthFirst < lengthSecond) ? lengthFirst : lengthSecond);
	compareLength = ((compareLength < maxCodePoints) ? compareLength : maxCodePoints);
